...
class TestNVMe(unittest.TestCase):
    ...
    def create_ns(self, nsze, ncap, flbas, dps):
        ...
        """
        create_ns_cmd = f"nvme create-ns {self.ctrl} --nsze={str(nsze)} --ncap={str(ncap)} --flbas={str(flbas)} --dps={str(dps)}"
    ...
    def attach_ns(self, ctrl_id, ns_id):
        ...
        """
        attach_ns_cmd = f"nvme attach-ns {self.ctrl} --namespace-id={str(ns_id)} --controllers={ctrl_id}"
    ...
    def detach_ns(self, ctrl_id, nsid):
        """ Wrapper for detaching the namespace.
            ...
            - Returns:
                - 0 on success, error code on failure.
        """
        detach_ns_cmd = f"nvme detach-ns {self.ctrl} --namespace-id={str(nsid)} --controllers={ctrl_id}"
    ...
    def run_ns_io(self, nsid, lbads):
        ...
        io_cmd = f"dd if={ns_path} of=/dev/null bs={str(block_size)} count=10 > /dev/null 2>&1"
        print(io_cmd)
        ...
        io_cmd = f"dd if=/dev/zero of={ns_path} bs={str(block_size)} count=10 > /dev/null 2>&1"
        print(io_cmd)
        run_io = subprocess.Popen(io_cmd, shell=True, stdout=subprocess.PIPE,
                                  encoding='utf-8')
        run_io_result = run_io.communicate()[1]
    ...
    def supp_check_id_ctrl(self, key):
        ...
        """
        id_ctrl = f"nvme id-ctrl {self.ctrl}"
        print("\n" + id_ctrl)
